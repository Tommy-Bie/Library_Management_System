...
class Ui_bookadmin(object):
    def setupUi(self, bookadmin):
        ...
        bookadmin.resize(846*1.9, 796)

        # 显示华南理工大学校徽
        lb1 = QLabel(self)
        lb1.setGeometry(860, 50, 1300, 700)
    ...
    def retranslateUi(self, bookadmin):
        ...
        bookadmin.setWindowTitle(_translate("bookadmin", "图书管理员系统 Book Management"))
    ...
    def bohui(self):
        s = self.iteminfo.currentRow()  # 获取当前申请信息
        bookid = self.iteminfo.item(s, 1).text()  # 获取该申请的书号
        sql = 'DELETE FROM item WHERE bookid="%s"' % (bookid)  # 从item表中删除
        cursor.execute(sql)
        conn.commit()
