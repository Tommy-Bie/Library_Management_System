...
class Ui_Reader(object):
    ...
    def setupUi(self, Reader):
        ...
        Reader.resize(719*2, 742)

        # 显示华南理工大学校徽
        lb1 = QLabel(self)
        lb1.setGeometry(730, 10, 1300, 700)
        lb1.setPixmap(QPixmap('scut2.jpg'))
    ...
    def retranslateUi(self, Reader):
        ...
        Reader.setWindowTitle(_translate("Reader", "读者系统 Reader"))
    ...
    # 提交借书申请
    def submit(self):
        s = self.borrowtable.currentRow()  # borrow表中的当前行
        if s == -1:  # 如果没有选择书就直接点击提交就会发出警告
            QMessageBox.warning(self, "警告", "请点击想借阅的书！", QMessageBox.Yes)
        else:
            remain = int(self.borrowtable.item(s, 4).text())  # 该书的在馆册数
            if remain == 0:  # 如果这本书没有了发出借光提示
                QMessageBox.warning(self, "警告", "这本书已经借光啦！", QMessageBox.Yes)
            else:
                t = self.borrowtable.item(s, 0).text()
                # 从borrow表中查询是否已经提交过相同申请
                sql = 'SELECT * FROM item where ID="%s" and bookid="%s" and type="borrow"' % (self.loginID, t)
                res = cursor.execute(sql)
                if res:  # 如果重复提交申请则发出警告
                    QMessageBox.warning(self, "警告", "请勿重复提交！", QMessageBox.Yes)
                else:
                    nowtime = time.strftime("%Y-%m-%d", time.localtime())
                    # 将借书的申请信息插入到item表中
                    sql = 'INSERT INTO item(bookid,ID,time,type) VALUES ("%s","%s","%s","borrow")' % (
                    t, self.loginID, nowtime)
                    cursor.execute(sql)
                    conn.commit()
